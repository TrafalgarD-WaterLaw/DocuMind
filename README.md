...
**切块策略的关键**:文物文档(图录/鉴赏文)天然高度结构化——一个器物条目 = 标题 + 简介 + 鉴赏分节 + 图片及图注。因此**按结构切而不是按字符数切**:标题、段落、表格各自成为独立**子块**(粒度细、命中准);同一器物的全部子块再拼接成一个**父块**(上限 1500 字,承载完整条目上下文)。检索命中子块时自动带上父块——既保住"精确到段"的粒度,又不丢"整条器物"的语境。
