...
- 💬 **快速问答** — 四通道混合检索(语义向量 / BM25 / Neo4j 图谱 / CLIP 图片)+ 证据锚定生成,端到端稳态检索延迟≤3s,回答携带 `[1][2]` 引用徽章,点击直达证据原文,图文并排对照(如「妇好墓玉龙是什么时期的玉器?」→ 商代、玉器、尺寸、出土信息带引用)
...
- 📊 **全链路可观测** — 每次检索各通道命中数与耗时实时流式推送诊断面板,jsonl 落盘可回溯
...
**1. 长尾文物知识,通用 RAG 召回差。** 文物是典型的"长尾领域"——同一个器物,用户会问年代、纹饰、工艺、出土、馆藏、文化意义,泛化问答模型对这类冷门实体经常答非所问、甚至编造。难点在于:知识分散在爬取文本、结构化字段、图片图注、图谱关系四种形态里,单一检索路无法覆盖。**所以**做四通道联合召回(语义向量 / BM25 / Neo4j 图谱 / CLIP 图片,内部增强含 Q-to-Q 问题索引与实体锚定)+ 加权 RRF 融合,让每一形态的知识都有独立通道被捞起,再统一融合排序。
...
**3. 用户问法与库内陈述句的语义鸿沟。** 库内是"妇好墓玉龙,商代,玉器,长7厘米…"的陈述句,用户问"这是什么时期的?"——embedding 距离远,BM25 靠关键词偶合。难点:查询侧增强(正向 HyDE)增加延迟与成本。**所以**采用**倒置 HyDE 范式**——在**入库侧**为每个 chunk 预生成 3 条假设问题,构建 16152 条问题索引,Q-to-Q 匹配,查询侧零额外延迟。
...
| 四通道召回 | 语义 / BM25 / 图谱 / CLIP 图片(内部增强:树剪枝、Q-to-Q 问题索引、实体锚定) | 知识分散在文本/字段/图注/图谱四种形态 |
| 问题索引 | 入库侧**对着答案生成问题**(Q-to-Q,倒置 HyDE 范式) | 正向 HyDE 用 LLM 猜答案查库,长尾下"猜错→查不准"循环 |
...
| 图谱枚举 | 枚举类问题图谱直出 + 强制注入融合列表头部(Priority Pinning) | 枚举结果曾被打散在语义结果里,确定性结果不靠 RRF 竞争 |
| 可靠性 | 模板化 Cypher 防注入 + 图谱不可达毫秒级静默降级(60s 熔断半开期)+ 启动预热 | 外部依赖故障被隔离,首问不被模型加载阻塞 |
...
> 知识库总量:9600+ 文本/图片块(含 3820 个图注级图片块)· 16152 条假设问题 · 9468 张图索引 · 2601 件文物 / 409 遗址 / 11 朝代 / 4 窑口(图谱)。运行数据不入库,重建脚本见 `backend/scripts/`(数据集目录经环境变量 `BRONZE_DATASET_DIR` / `PORCELAIN_DATASET_DIR` 配置);上传 PDF 走统一入库管道 `python -m ingestion --source X`。

## 评测

> 检索改动强制回归防退化(`cd backend && uv run python eval/run_eval.py --retrieval-only`),评测脚本与报告见 `backend/eval/`。

| 维度 | 指标 | 结果 |
|---|---|---|
| 检索(核心评测集) | Recall@8 / MRR | **96% / 0.76**(自建 100 题六类人工精标:实体名检索 / 去实体名特征 / 多轮消解 / 多源枚举 / 混淆区分 / 边界,难题型占比近半,含图谱枚举与多模态用例) |
| 生成(LLM-as-judge 四维) | Context Recall / Context Precision / Faithfulness / Answer Relevancy | **0.92 / 0.85 / 0.86 / 0.80** |
| 混合模态(图+文联合检索) | AnyHit@8 | **15/15** |
| 文找图 | AnyHit@5 | **12/13**(唯一失败为釉色视觉混淆,属设计内权衡) |
...
| 向量检索 | ChromaDB · bge-small-zh · BM25(jieba + rank_bm25) |
| 检索融合 | 加权 RRF(k=60)· 权重网格实验定标 · RRF 分数分界噪声过滤 |
| 知识图谱 | Neo4j · neomodel ODM |
| 多模态 | Chinese-CLIP(图文互检)· Qwen-VL(图注)· Docling(PDF 解析) |
| LLM | DeepSeek(NDJSON 流式) |
| 评测 | 自建 100 题六类评测集 · LLM-as-judge 四维评测 |

## 未来规划

- [ ] **多源知识接入** — 更多数据源经统一 ingest 管道接入(契约校验已就绪,新源即插即用),支持扫描件 OCR、长文档分层切块
- [ ] **检索能力扩展** — 多跳检索(图谱多级扩展)、检索策略按意图自适应
- [ ] **多轮记忆增强** — 从 LRU 摘要升级为分层记忆(短期事实 / 长期偏好),长对话不丢上下文
- [ ] **Agent 进化** — 专家 Agent 工具调用(查图 / 查图谱 / 查外部知识),多模态 Agent 协作
- [x] **评测闭环** — 自建 100 题六类评测集 + LLM-as-judge 四维评测,检索改动强制回归(详见[评测](#评测))
- [ ] **最终愿景:文物知识开放平台** — 把"拍照即问、问答有据"沉淀为面向公众的文物数字服务
