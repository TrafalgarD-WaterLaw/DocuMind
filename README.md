...
**1. 长尾文物知识,通用 RAG 召回差。** 文物是典型的"长尾领域"——同一个器物,用户会问年代、纹饰、工艺、出土、馆藏、文化意义,泛化问答模型对这类冷门实体经常答非所问、甚至编造。难点在于:知识分散在爬取文本、结构化字段、图片图注、图谱关系四种形态里,单一检索路无法覆盖。**所以**做六路混合召回 + 加权 RRF 融合,让每一形态的知识都有独立通道被捞起,再统一融合排序(检索契约 Recall@8 = 28/28)。
...
| 难点 | 方案与权衡 | 效果 |
|---|---|---|
| 六路检索质量参差,权重怎么定 | **网格实验定标**,不拍脑袋:图谱路与语义重叠降权 0.5、CLIP"视觉相似≠语义相关"降至 0.1、图注块 0.3 不与文本同权 | 检索契约 Recall@8 = 100%(28/28),MRR 0.693 |
| 弱证据块污染 LLM 上下文 | 依据 RRF 分数分布(多路票 ≥0.0308 / 单票 ≤0.0167 的天然分界 0.025)做**块级噪声过滤** | 噪声块裁剪率约 50%,上下文质量显著提升 |
| 图谱查询易注入、易崩 | **模板化 Cypher(T1-T6,不自由生成)** + 失败 60s 冷却 + 静默降级文本检索 | 图谱宕机时检索契约仍全绿,演示不中断 |
| 重模型加载阻塞首问 | 启动**后台预热**(BM25/CLIP/embedder 全量就绪),请求零等待 | 首问不卡 1 分钟级模型加载(实测预热后即问即答) |
| 精排要不要上 reranker | 完整网格实验后**维持禁用**:bge-reranker 全面退化(Recall -7.1pp / MRR -14%),4GB 消费级显存不可外推 | 负面结论同样可解释——候选池 64 无精排更优 |
