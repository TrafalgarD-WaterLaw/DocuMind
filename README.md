...
- `bm25` **关键词检索** — jieba 分词 + rank_bm25,5786 篇文档,脏标记惰性重建
- `graph` **图谱锚定** — LLM 从问题提实体 → Neo4j 查关联(2601 件文物 / 409 遗址 / 11 朝代 / 4 窑口,2803 条出土关系)→ 扩展词补检索
- `entity` **实体名精确匹配** — 入库时注入的实体数组按名精确匹配,补偿短条目 embedding 劣势
- `clip` **文找图 / 图找图** — CLIP 图文同空间,9079 张图索引:文找图(文本 query 按视觉相似召回,参与六路融合,低权重防挤占,见链路 5);图找图(vision 专用通道——上传照片按图-图余弦识别出库内相似文物,识别名再驱动六路检索,见演示·拍照识物)

**效果**:六路各补一个形态短板,覆盖"文本 / 字段 / 图注 / 图谱"四态知识;诊断面板实时展示每路命中数与耗时,哪路捞到证据一目了然。
