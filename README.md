...
### 链路 1 · 文档入库:结构感知解析 + 原子替换

**难点**:PDF 布局复杂——表格、图片、多栏混排,粗暴按字符切分会把表格拆碎、把图片信息丢掉;解析失败(加密/扫描件/损坏)需要机器可读的失败分类,而不是让用户猜。

**方案**:Docling 结构感知解析(段落 / 标题 / 表格独立块 + 父块拼接),图片抽取 + VLM 图注(无 key 降级文件名占位);sha256 指纹去重,重复上传直接跳过;**replace 模式"先建后删"**——新块全部入库成功后才删旧块,失败不动旧数据。

**效果**:上传 PDF 秒级可检索,重复上传零重复块,解析失败返回机器可读分类(加密/权限/超时/无效 PDF),前端可映射展示。
