...
**方案**:六路并行召回,每路补一个形态短板:
- `semantic` 向量检索 + **树状层级剪枝**(窑口→器物→鉴定维度逐层粗筛,候选池从全库收窄到每层 top-N,省计算)
- `question` Q-to-Q 问题索引 · `bm25` 关键词 · `graph` 图谱锚定 · `entity` 实体名精确匹配(短条目 embedding 劣势补偿)· `clip` 文找图

**效果**:召回面覆盖四态知识,检索契约 Recall@8 = 28/28 = 100%。
