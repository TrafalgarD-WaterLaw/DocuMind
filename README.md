...
**难点**:六路召回质量参差——图谱扩展词与语义路高度重叠、CLIP"视觉相似"常不等于"语义相关"(汝窑天青釉与钧窑天兰釉连人眼都难分)、图注块的语义相似度升高还会挤占文本证据。

**方案**:把权重当作可实验参数跑网格实验:graph 路降权 0.5(与 semantic 重叠)、clip 路压到 0.1(防视觉噪声挤占文本)、图注块 0.3(文本证据弱时可浮上,但不与文本同权竞争)。

**效果**:检索契约 Recall@8 = 28/28 = 100%、MRR 0.693——每个权重都有实验出处,而非拍脑袋。

### 2. 假设性问题索引:把"问法鸿沟"解决在入库侧
...
**权衡**:与查询侧增强(HyDE)对比——HyDE 每次查询多一次 LLM 生成(增延迟、增成本),我们**预生成**,查询侧零额外开销,代价是入库成本一次性付清。

**效果**:question 路成为稳定召回来源(每轮 3-6 条命中),且对通用问题形态的加权结论同样来自实验(权重 1.0,不特殊加权)。

### 3. 块级噪声过滤:用 RRF 分数分布找到天然分界

**难点**:混合检索把弱证据块(仅单路低分命中)也送进 LLM 上下文——多路票的高分块与单路票的弱块混在一起,稀释回答质量。

**方案**:统计真实检索的 RRF 分数分布,发现多路票 ≥0.0308、单路票 ≤0.0167,天然分界约 0.025——低于此分且单票、排名靠后的弱块直接裁剪,不送 LLM。

**效果**:噪声块裁剪率约 50%,上下文从"8 块 × 全量"收窄到高置信证据,回答引用质量显著提升。

### 4. 图谱查询模板化 + 结构化降级

**难点**:图谱问答直接让 LLM 生成 Cypher——注入风险 + 语法错误率高;且 Neo4j 是外部依赖,宕机时问答链路不能断。

**方案**:预置模板化 Cypher(T1-T6:按意图路由,不自由生成);Neo4j 连接失败进入 60s 冷却期,快速失败并**静默降级文本检索**。

**效果**:实测图谱完全宕机时检索契约仍 28/28 全绿——外部依赖的故障被隔离在可观测的降级路径内。

### 5. reranker 实验的负面结论:维持禁用

**难点**:RAG 范式里 cross-encoder 精排几乎是标配,不上显得"不完整"。

**方案**:对 bge-reranker-v2-m3 做完整网格实验(候选池 / 召回数 / 每路 K 全组合)。

**权衡与结论**:文本场景全面退化(Recall -7.1pp / MRR -14%)——精排对已有多路投票 + RRF 的融合收益为负;多模态场景受 4GB 消费级显存限制无法评估,不可外推。

**效果**:基于实验结论**维持禁用**,候选池 64 无精排更优——敢于记录并采信负面结果,比"加了个 reranker"更有工程说服力。
